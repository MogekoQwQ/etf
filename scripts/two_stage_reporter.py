...
from traditional_model_config import (
    ...
    get_traditional_two_stage_report_file,
)
...
PARAMETER_LABELS = {
    ...
    "holding_days": "持有天数",
    "use_llm": "是否启用 LLM 两阶段排序",
    "mock_llm": "是否使用 Mock 验证模式",
...
BACKTEST_PREVIEW_LABELS = {
    "rebalance_date": "调仓日期",
    "strategy_a_codes": "基线策略持仓",
    "strategy_b_codes": "两阶段策略持仓",
    "strategy_a_return": "基线策略收益",
    "strategy_b_return": "两阶段策略收益",
    "strategy_a_count": "基线策略持仓数",
    "strategy_b_count": "两阶段策略持仓数",
    "llm_ranking_used": "是否使用 LLM 排序",
    "llm_fallback_used": "是否触发回退",
    "explanation_requested": "是否请求解释",
    "explanation_generated": "是否生成解释",
}
...
def match_alias(value: Any, alias_map: Dict[str, List[str]]) -> Optional[str]:
    ...
    # Prefer exact matches to avoid broad substring collisions such as
    # `strategy_b` being captured by the `a` alias from `strategy_a`.
    ...
    for canonical, aliases in alias_map.items():
        ...
        for alias in candidates:
            ...
            if len(alias_normalized) < 3 or len(normalized) < 3:
                continue
...
class TwoStageReporter:
    ...
    def get_strategy_display_names(self, report_data: Dict[str, Any]) -> Tuple[str, str]:
        parameters = report_data.get("parameters", {}) if isinstance(report_data, dict) else {}
        baseline_label = "传统模型基线策略"
        if bool(parameters.get("mock_llm")):
            return baseline_label, "Mock 两阶段验证策略"
        if bool(parameters.get("use_llm")):
    ...
    def load_metrics_comparison(self) -> Dict[str, Dict[str, Optional[float]]]:
        ...
        if os.path.exists(self.metrics_path):
            try:
                metrics_df = read_csv_with_fallback(self.metrics_path)
        ...
        for metric_key, label, _ in DISPLAY_METRICS:
            if is_missing(comparison["traditional"].get(metric_key)) or is_missing(comparison["two_stage"].get(metric_key)):
                self.warnings.append(f"绩效指标“{label}”缺少完整数据，报告中将显示为 N/A。")
    ...
    def load_backtest_preview(self) -> pd.DataFrame:
        ...
        try:
            frame = read_csv_with_fallback(self.backtest_path)
        except Exception as exc:
        ...
        for column in preview.columns:
            ...
            elif preview[column].dropna().isin([True, False]).all():
                preview[column] = preview[column].map(lambda value: "是" if value else "否")
        preview = preview.rename(columns={col: BACKTEST_PREVIEW_LABELS.get(col, col) for col in preview.columns})
        return preview
    ...
    def build_html(
        ...
        baseline_label, two_stage_label = self.get_strategy_display_names(report_data)
...
<body>
    <div class="header">
        <h1>ETF 单模型两阶段回测报告</h1>
        <div class="subtitle">{html.escape(baseline_label)}与{html.escape(two_stage_label)}的回测结果对比</div>
        <div class="meta">
        ...
        <div class="table-wrapper">
            <table class="report-table">
                <thead>
                    <tr>
                        <th>指标</th>
                        <th>{html.escape(baseline_label)}</th>
                        <th>{html.escape(two_stage_label)}</th>
                    </tr>
                </thead>
                <tbody>
                    {self.build_metric_rows(comparison, baseline_label, two_stage_label)}
                </tbody>
            </table>
        </div>
    </div>
    ...
    <div class="section">
        <h2>回测明细示例</h2>
        <p>以下表格展示 `backtest_results.csv` 的前 10 行，并对展示列做了中文映射，便于快速阅读。</p>
        {backtest_table}
    </div>
    ...
    <div class="footer">
        <p>本报告由 ETF 两阶段研究框架自动生成，仅用于结果展示与复核。</p>
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--input-dir",
        type=str,
        default=get_traditional_two_stage_output_dir(PROJECT_ROOT, DEFAULT_TRADITIONAL_MODEL, "deepseek-chat"),
        help="两阶段回测结果目录",
    )
    parser.add_argument(
        "--output",
        type=str,
        default=get_traditional_two_stage_report_file(PROJECT_ROOT, DEFAULT_TRADITIONAL_MODEL, "deepseek-chat"),
        help="HTML 汇总报告输出路径",
    )
