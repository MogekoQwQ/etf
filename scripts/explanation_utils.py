...
class ExplanationStorage:
    ...
    def __init__(self, base_dir: str):
        ...
        Args:
            base_dir: 基础目录路径，如 "./results/traditional_models/<traditional_model>/two_stage/llm_logs"
        """
    ...
    def _save_summary_text(self, date: str, result: Dict[str, Any], summary_path: str):
        ...
        # 添加日期标题
        summary_content.append(f"=== ETF LLM 排序解释报告 ===\n")
        ...
        if "rankings" in result:
            ...
            for i, item in enumerate(result["rankings"][:3], 1):
                ...
                summary_content.append(f"{i}. {code}（LLM 两阶段评分：{score:.3f}）")
                if explanation:
                    summary_content.append(f"   解释说明：{explanation}")
    ...
    def _update_index(self, date: str, date_dir: str, etf_data: pd.DataFrame):
        ...
        if os.path.exists(index_path):
            index_df = read_csv_with_fallback(index_path)
        else:
...
if __name__ == "__main__":
    # 创建存储管理器
    storage = ExplanationStorage("./results/traditional_models/<traditional_model>/two_stage/llm_logs")
