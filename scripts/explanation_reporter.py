...
from traditional_model_config import DEFAULT_TRADITIONAL_MODEL, get_traditional_explanation_dir
...
class ExplanationReporter:
    ...
    def _generate_html_content(self, date: str, data: Dict[str, Any]) -> str:
        ...
        market_context = html.escape(str(summary.get("market_context", "LLM 基于当日因子数据生成的市场环境判断。")))
...
<head>
    ...
    <title>ETF LLM 排序解释报告 - {date}</title>
    <style>
        body {{
...
<body>
    <div class="header">
        <h1>ETF LLM 排序解释报告</h1>
        <div class="subtitle">LLM 两阶段策略的候选 ETF 排序解释 | 日期：{date}</div>
    </div>
    ...
    <div class="section">
        ...
        <div class="metrics-container">
            <div class="metric-card">
                <div class="metric-label">候选 ETF 数量</div>
            ...
            <div class="metric-card">
                <div class="metric-label">解释来源</div>
...
{key_factor_tags}
    ...
    <div class="section">
        <h2>候选 ETF 排序解释</h2>
        <p>以下展示 LLM 两阶段策略排序前 10 名 ETF 及其解释。</p>
...
{chr(10).join(rows)}
    ...
    <div class="section">
        ...
        <p><strong>LLM 解释的风险考虑：</strong> {risk_considerations}</p>
    </div>

    <div class="footer">
        <p>报告生成系统：ETF 两阶段研究框架 - LLM 排序解释模块</p>
    ...
    def generate_summary_report(self, start_date: Optional[str] = None,
        ...
        for date_str, date_dir in date_dirs:
            ...
            report_link = (
                f'<a href="{self._report_href(report_path)}">查看 HTML 报告</a>'
...
<body>
    <div class="header">
        <h1>ETF LLM 排序解释汇总报告</h1>
        <p>日期范围：{start_date or '最早'} 至 {end_date or '最新'} | 共 {len(date_dirs)} 个交易日</p>
    </div>
{''.join(cards)}
...
def main():
    parser = argparse.ArgumentParser(description="生成 ETF LLM 排序解释报告")
    parser.add_argument("--date", type=str, help="生成指定日期的报告（格式：YYYY-MM-DD）")
    parser.add_argument("--start-date", type=str, help="汇总报告起始日期")
    parser.add_argument("--end-date", type=str, help="汇总报告结束日期")
    parser.add_argument(
        "--explanations-dir",
        type=str,
        default=get_traditional_explanation_dir(PROJECT_ROOT, DEFAULT_TRADITIONAL_MODEL),
        help="解释结果目录路径",
    )
    ...
    try:
        if args.date:
            ...
            print(f"[OK] 单日解释报告生成成功: {output_path}")
        else:
            output_path = reporter.generate_summary_report(args.start_date, args.end_date)
            print(f"[OK] 解释汇总报告生成成功: {output_path}")
    except Exception as exc:
        print(f"[FAIL] 解释报告生成失败: {exc}")
        import traceback
        traceback.print_exc()
        raise SystemExit(1)
